...
fn event_bus_emit_appends_to_log() {
    ...
    let id =
        bus.emit(ActorId::local_user(), target, EventKind::Lifecycle(LifecycleKind::Created), None);
...
fn event_bus_causation_links() {
    ...
    let child_id =
        bus.emit(actor.clone(), t, EventKind::Lifecycle(LifecycleKind::Destroyed), Some(parent_id));
