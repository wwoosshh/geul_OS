...
impl ObjectServer {
    ...
    pub fn set_state(
        ...
        // 2) ACL — M11: invoke와 동일한 is_allowed 평가 경로. owner는 ACL 비어있을 때만
        // short-circuit (Object::is_allowed 내부에서 처리).
        if !obj.is_allowed(actor, crate::object::AclOp::SetState(key.to_string()), &self.grants) {
